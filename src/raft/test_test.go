...
func TestReElection2A(t *testing.T) {
	...
	DPrintf("leader %v: starts disconnect", leader1)
	cfg.disconnect(leader1)
	DPrintf("leader %v: finishes disconnect", leader1)
	new_leader := cfg.checkOneLeader()
  DPrintf("libo2, new leader: %v", new_leader)
	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()
	DPrintf("libo3, new leader: %v", leader2)

	// if there's no quorum, no leader should
	// be elected.
	cfg.disconnect(leader2)
	cfg.disconnect((leader2 + 1) % servers)
	DPrintf("discover two servers")
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()
	DPrintf("no leaders found")
